<?php
    ...
    //find number of dislikes
    $stmt = $mysqli->prepare("SELECT likes from likes where article_name=?");
    if(!$stmt){
    ...
    //set disliked field in user specific table to be true
    //IF THE USER ALREADY HAS THE ARTICLE, CHANGE DISLIKED FIELD IN SPECIFIC USER TABLE TO BE 'Y'
    if ($article_present==1){
        $yes = 'y';
        $stmt5 = $mysqli->prepare("UPDATE $username set disliked=? where article_id = ?");
        if(!$stmt5){
            printf("Query Prep Failed: %s\n", $mysqli->error);
            exit;
        }
        ...
        $stmt6->bind_param('sssss', $article_id, $no, $no, $no, $yes);
        $stmt6->execute();
        $stmt6->close();
    }
